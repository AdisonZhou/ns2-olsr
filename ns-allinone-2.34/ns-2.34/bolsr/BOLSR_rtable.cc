...
///
BOLSR_rtable::~BOLSR_rtable() {
	// Iterates over the routing table deleting each BOLSR_rt_entry*.
...
///
void
BOLSR_rtable::rm_entry(nsaddr_t dest) {
...
///
BOLSR_rt_entry*
BOLSR_rtable::lookup(nsaddr_t dest) {
...
///
BOLSR_rt_entry*
BOLSR_rtable::find_send_entry(BOLSR_rt_entry* entry) {
	BOLSR_rt_entry* e = entry;
...
///
BOLSR_rt_entry*
BOLSR_rtable::add_entry(nsaddr_t dest, nsaddr_t next, nsaddr_t iface, u_int32_t dist,double rute_score2) {
	// Creates a new rt entry with specified values
	BOLSR_rt_entry* entry = new BOLSR_rt_entry();
	entry->dest_addr()	= dest;
	entry->next_addr()	= next;
	entry->iface_addr()	= iface;
	entry->dist()		= dist;
	entry->rute_score()=rute_score2;
...
///
void
BOLSR_rtable::print(Trace* out) {	
	sprintf(out->pt_->buffer(), "P\tdest\tnext\tiface\tdist");
	out->pt_->dump();
	for (rtable_t::iterator it = rt_.begin(); it != rt_.end(); it++) {
		BOLSR_rt_entry* entry = (*it).second;
		sprintf(out->pt_->buffer(), "P\t%d\t%d\t%d\t%d",
			BOLSR::node_id(entry->dest_addr()),
			BOLSR::node_id(entry->next_addr()),
			BOLSR::node_id(entry->iface_addr()),
			entry->dist());
		out->pt_->dump();
	}
}
