...
///
void
AOLSR_printer::print_cmn_hdr(FILE* out, struct hdr_cmn* ch) {
...
///
void
AOLSR_printer::print_ip_hdr(FILE* out, struct hdr_ip* ih) {
	fprintf(out,
		"  IP HEADER\n\tsrc_addr = %d\n\tsrc_port = %d\n\tdest_addr = %d\n\tdest_port = %d\n\tttl = %d\n\n",
		ih->saddr(),
		ih->sport(),
...
///
/// \brief Prints a given AOLSR packet into a given file.
/// \warning This function is actually not used.
/// \param out	File where the %AOLSR packet is going to be written.
/// \param pkt	%AOLSR packet to be written.
///
void
AOLSR_printer::print_aolsr_pkt(FILE* out, AOLSR_pkt* pkt) {
...
///
/// \brief Prints a given %AOLSR message into a given file.
/// \warning This function is actually not used.
/// \param out	File where the %AOLSR message is going to be written.
/// \param msg	%AOLSR message to be written.
///
void
AOLSR_printer::print_aolsr_msg(FILE* out, AOLSR_msg& msg) {
	char *msg_type;

	switch (msg.msg_type()) {
	case AOLSR_HELLO_MSG:
		msg_type = "HELLO";
		break;
	case AOLSR_TC_MSG:
		msg_type = "TC";
	...
	fprintf(out,
		...
		"msg_seq_num = %d\n",
		msg_type,
		AOLSR::emf_to_seconds(msg.vtime()),
		msg.msg_size(),
		msg.orig_addr(),
		msg.ttl(),
		msg.hop_count(),
	...
	if (msg.msg_type() == AOLSR_HELLO_MSG)
		print_aolsr_hello(out, msg.hello());
	else if (msg.msg_type() == AOLSR_TC_MSG)
		print_aolsr_tc(out, msg.tc());
	else if (msg.msg_type() == AOLSR_MID_MSG)
		print_aolsr_mid(out, msg.mid());
}

///
/// \brief Prints a given %AOLSR HELLO message into a given file.
/// \warning This function is actually not used.
/// \param out		File where the %AOLSR HELLO message is going to be written.
/// \param hello	%AOLSR HELLO message to be written.
///
void
AOLSR_printer::print_aolsr_hello(FILE* out, AOLSR_hello& hello) {

	fprintf(out, "\thtime = %.2f\n\twillingness = %d\n",
		AOLSR::emf_to_seconds(hello.htime()),
		hello.willingness());

	for (int i = 0; i < hello.count; i++) {
		char *nt, *lt;
		AOLSR_hello_msg msg = hello.hello_msg(i);

		u_int8_t nb_type = msg.link_code() >> 2;
		u_int8_t link_type = msg.link_code() & 0x03;
		if (nb_type == AOLSR_NOT_NEIGH)
			nt = "NOT_NEIGH";
		else if (nb_type == AOLSR_SYM_NEIGH)
			nt = "SYM_NEIGH";
		else if (nb_type == AOLSR_MPR_NEIGH)
			nt = "MPR_NEIGH";
		else
			nt = "UNKNOWN (likely a bug!)";

		if (link_type == AOLSR_UNSPEC_LINK)
			lt = "UNSPEC_LINK";
		else if (link_type == AOLSR_ASYM_LINK)
			lt = "ASYM_LINK";
		else if (link_type == AOLSR_SYM_LINK)
			lt = "SYM_LINK";
		else if (link_type == AOLSR_LOST_LINK)
			lt = "LOST_LINK";
		else
...
///
/// \brief Prints a given %AOLSR TC message into a given file.
/// \warning This function is actually not used.
/// \param out	File where the %AOLSR TC message is going to be written.
/// \param tc	%AOLSR TC message to be written.
///
void
AOLSR_printer::print_aolsr_tc(FILE* out, AOLSR_tc& tc) {
	fprintf(out, "\tansn = %d\n\treserved = %d\n",
		tc.ansn(),
		tc.reserved());
...
///
/// \brief Prints a given %AOLSR MID message into a given file.
/// \warning This function is actually not used.
/// \param out	File where the %AOLSR MID message is going to be written.
/// \param mid	%AOLSR MID message to be written.
///
void
AOLSR_printer::print_aolsr_mid(FILE* out, AOLSR_mid& mid) {
...
///
void
AOLSR_printer::print_mprset(Trace* out, mprset_t& mprset) {
	sprintf(out->pt_->buffer(), "P\tnb");
	out->pt_->dump();
	for (mprset_t::iterator it = mprset.begin(); it != mprset.end(); it++) {
		sprintf(out->pt_->buffer(), "P\t%d", AOLSR::node_id(*it));
		out->pt_->dump();
	}
}
...
///
void
AOLSR_printer::print_mprselset(Trace* out, mprselset_t& mprselset) {
	...
	for (mprselset_t::iterator it = mprselset.begin(); it != mprselset.end(); it++) {
		...
		sprintf(out->pt_->buffer(), "P\t%d\t%f",
			AOLSR::node_id(mprsel_tuple->main_addr()),
			mprsel_tuple->time());
		out->pt_->dump();
	}
}
...
///
void
AOLSR_printer::print_topologyset(Trace* out, topologyset_t& topologyset) {
	...
	for (topologyset_t::iterator it = topologyset.begin(); it != topologyset.end(); it++) {
		...
		sprintf(out->pt_->buffer(), "P\t%d\t%d\t%d\t%f",
			AOLSR::node_id(topology_tuple->dest_addr()),
			AOLSR::node_id(topology_tuple->last_addr()),
			topology_tuple->seq(),
			topology_tuple->time());
		out->pt_->dump();
	}
